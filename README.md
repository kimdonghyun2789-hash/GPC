...
- **식당 DB 관리** — 추가/수정/비활성화, 엑셀 업로드·붙여넣기(엑셀/시트 복사), 샘플 데이터 추가.
- **방문 기록 붙여넣기** — 과거 방문 내역을 엑셀/시트에서 복사해 대량 등록(없는 식당 자동 생성).
...
- **팀 점심 모드** — 참석 인원·제외 메뉴로 후보를 뽑아 투표(빠른/랜덤 결정), 결과 공유, 팀 선호도 분석.
- **고정 팀원 프로필** — 팀원과 비선호 메뉴를 등록하면 참석자 체크만으로 해당 메뉴를 자동 제외.
...
- **내 취향 설정** — 선호/비선호 메뉴·태그를 등록하면 추천 점수에 반영.
- **좌표 기반 도보시간 자동 계산** — 기준 위치에서 식당 좌표까지 거리로 도보시간 추정.
- **AI 기능(선택)** — 자연어 요청 분석, 추천 코멘트, 오늘의 한줄 코멘트, 방문 메모 분석, 태그 자동 추천, 식당 요약, 예산 조언, 취향 분석, 월별 리포트.
