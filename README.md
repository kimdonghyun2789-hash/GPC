...
- **한동안 그만 보기 / 별로였어요** — 30일 추천 제외, 또는 식당·카테고리 선호 감점 피드백.
- **지출 분석** — 카테고리별 지출, 비싼 식당·가성비 식당 TOP 5.
- **팀 점심 모드** — 참석 인원·제외 메뉴로 후보를 뽑아 투표(빠른/랜덤 결정), 팀 선호도 분석.
- **네이버 식당 자동 수집(선택)** — 회사 주소를 넣으면 네이버 지역 검색으로 주변 식당을 키워드별로 자동 수집/병합.
- **상황별 추천 모드** — 빠른 점심·든든하게·가성비·새로운 곳·해장·국물·면 등 한 번 탭으로 추천 기준 변경.
